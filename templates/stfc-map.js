...
STFCMap = (function() {
    ...
    const versionNumber = '1.0.0'; //info.version
    const xMin = -6357;
    const xWidth = 4400;
    ...
    let assetsUrl = '.'; //default assets folder, if defined will load assets from an external source
    ...
    let initSystems = function(_galaxy) {
        ...
        $.getJSON('./marked_systems.json', function(responseObject){
    ...
    let makeSystemNode = function(sys) {
        ...
        if(iconType === '') {
            ...
            node = makeCircle(coords, {
                ...
                id: sysName,
                radius: radius,
                color: sysName == "Hann" || sysName == "Gemet" ? "red" : '#fcf8e5',
                fillOpacity: 1,
                stroke: true,
    ...
    let makeSystemPopup = function(d) {
        ...
        let info =
            `<div id="system-zone">${territory}&nbsp;<span id="system-event">${event}</span></div>
             <div id="system-name"><button type="button" onclick="alert('You pressed the button!')">Click me!</button>${name} [${systemLevel}]</div>
    ...
    let setAttributions = function(info) {
        let mapLink = "<a href='https://stfcmap.org' title='Star Trek Fleet Command Galaxy Map'>";
        let discLinkA = "<a href='https://discord.com/invite/fKThyH2' title='STFC Pro Discord'>";
        let close = "</a>";
        let serverInfo = '[152] TFD'; //info.serverInfo
        let mapName = 'Star Trek Fleet Command Galaxy Map'; //info.mapName
        let author = 'TomcatDriver'; //info.author
        return mapLink + mapName + close + " v" + versionNumber + "<br>" + "By: <strong>" + author + "</strong> - Server: <strong>" + serverInfo + "</strong><br>" + discLinkA + "Based on STFCPro Map" + close;
    };
